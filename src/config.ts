...
export function getUpsConfig(): UpsConfig {
  ...
  const oauthTokenUrl =
    env("UPS_OAUTH_TOKEN_URL") ?? "https://onlinetools.ups.com/security/v1/oauth/token";
...
export function validateUpsConfig(config: UpsConfig): void {
  if (!config.clientId?.trim() || !config.clientSecret?.trim()) {
    throw new Error(
      "UPS_CLIENT_ID and UPS_CLIENT_SECRET must be set for UPS carrier. " +
